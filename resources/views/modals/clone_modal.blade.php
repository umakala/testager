          ...
          <div class="modal-body" style="padding-bottom: 10px">
              <div class="form-group"> 
                  ...
                  @elseif($clone_type == 'functionality')
                      <select class="form-control" name="tf_id"  style=" "> 
                          <option value="none">Select {{$clone_type}}</option>
                          @foreach ($clone_fn as $fn)    
                              <option value="{{$fn->tf_id}}" title="{{$fn->description}}" style= 'max-width: 100px;overflow: hidden;'> {{$fn->tf_name}} -
                                {{substr($fn->description, 0, 50).".."}}  </option>
                          @endforeach 
                      </select>
                  @endif
